<x-app-admin-layout>
            ...
            <form action="{{ route('product.bulkaction') }}" method="post">
                ...
                <div class="relative overflow-hidden bg-white shadow-md dark:bg-gray-800 sm:rounded-lg">
                    <div
                        ...
                        <x-create-model :categories="$categories">product</x-create-model>

                        <select id="bulkActionSelect" name="action"
                        ...
                        <button type="submit" id="bulkActionSubmit"
                            class="hidden items-center justify-center flex-shrink-0 px-3 py-2 text-sm font-medium text-gray-900 bg-white border border-gray-200 rounded-lg focus:outline-none hover:bg-gray-100 hover:text-primary-700 focus:z-10 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">
                            Apply
                        </button>
                    ...
                    <div
                        ...
                        <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                            ...
                            <tbody id="product-table-body">
                                @include('partials.product-table-rows', [
                                    'products' => $products,
    ...
    @push('scripts')
        <script>
            ...
            function checkbox() {
                const checkboxAll = document.querySelector('#checkbox-all');
                const checkboxProductTables = document.querySelectorAll('.checkbox-table-search');
                const totalAllCheckbox = checkboxProductTables.length;

                checkboxAll.addEventListener('change', function() {
                    const isChecked = this.checked;
                    checkboxProductTables.forEach(checkboxProduct => {
                ...
                checkboxProductTables.forEach(checkboxProduct => {
                    checkboxProduct.addEventListener('change', function() {
                        const totalCheckbox = document.querySelectorAll('.checkbox-table-search:checked')
                            .length;
                        checkboxAll.checked = totalAllCheckbox === totalCheckbox;
                    })
                })
            }

            function bulkAction() {
                const bulkActionSelect = document.querySelector('#bulkActionSelect');
                const bulkActionSubmit = document.querySelector('#bulkActionSubmit');
                bulkActionSelect.addEventListener('change', function() {
                    if (bulkActionSelect.value === '') {
                        bulkActionSubmit.classList.add('hidden');
                    } else {
                        bulkActionSubmit.classList.remove('hidden');
                    }
                })
